...
In order to simplify all computations, IIT assumes the following theorem to hold.

\begin{theorem}
	\label{thm:cond_independence}
...
We can convert $\mathbf{T}$ back to $\mathbf{M}$ if needed because $\mathbf{M} = p(Z_{t+1} = j| Z_t = i)$, due the the bijection between distributions and \textit{generators}
