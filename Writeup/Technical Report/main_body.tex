...
\begin{equation}
\label{def:phi2}
\varphi_{cause}^{M,N}(W_t=X) = D \left( p_c(Z_{t-1}|W_t=X) ,p(Z_{t-1}|W_{t} = X,M,N)  \right)
\end{equation}.
...
\begin{equation}
\label{def:phi3}
\varphi_{cause}^{\text{MIP}}(W_t=X)  = \varphi_{cause}^{M',N'}(W_t=X) 
\end{equation}
...
\begin{equation}
\label{def:phi}
\varphi^{\text{MIP}}(W_t=X) = \min \left( \varphi_{cause}^{\text{MIP}}(W_t=X), \varphi_{effect}^{\text{MIP}}(W_t=X)  \right) 
\end{equation}
...
\varphi^{\text{MIP}}(W_t=X) \leq cei(W_t=X) 
\end{equation}
...
We identify the \textit{core effect} and calculate $\varphi^{\text{Max}}_{\text{effect}}$ in the equivalent fashion, and finish with setting $\varphi^{\text{Max}}$ to be the minimum of the two as before. It is important to note that we find the core cause and effect separately, and that these need not be the same set.
...
\varphi^{\text{MIP}}(G^{p,f}|G^c = X) \leq \varphi^{\text{Max}}(G^c = X)\leq cei(G^{p,f}|G^c = X)
\end{equation}
