...
\begin{tabularx}{\textwidth}{ll X}
	...
	power set & $\mathbb{P}(W)$ & The power set of a \textit{mechanism} (or \textit{system}) is the set of all subsets of that \textit{mechanism}.\\
		\hline
	forward distribution & $p(W_{t+1}|V_t=X)$ & Let $W, V \subset Z$ be \textit{mechanisms}. We denote $p(W_{t+1}|V_t= X)$ as the probability distribution of possible future states of $W$ given that the current state $V$ is $X$. $p$ returns the probability of each possible future state occurring There are $2^{|W|}$ such states, and hence this is the size of the forward distribution.\\
		\hline
	backward distribution & $p(W_{}=X|V_{t-1})$ & Let $W, V \subset Z$ be \textit{mechanisms}. We denote$p(W_{t+1}=X|V_t)$ as the probability distribution of possible past states of $V$ given that the current state $W$ is $X$. Similarly to above, the size of the backward distribution is $2^{|V|}$\\
		\hline
	...
	\hline
	\textit{state} & $\mathbf{s}$ & Let $W$ be a $m$th order mechanism. We write a possible \textit{state} of $W$ as a row vector $\mathbf{s} \in [0,1]^m$.\\
	\hline
	\textit{state set} & $\mathbb{S}(W)$ & A $m$th order \textit{mechanisms} \textit{state set} is the set of all possible \textit{states} of that \textit{mechanism}. This size of this set is $2^m$.
\end{tabularx}
...
\label{sec:cost}
In this section, we construct each of the previously defined repertoires from the TPM. For later use, we define the product of probability distributions.
...
\begin{definition}
	...
	In other words the product of the distributions is the distribution for 
\end{definition} 

\subsubsection{Conditional Independence}
\label{sec:conditional_independence}
In order to simplify all computations, IIT makes a large mathematical assumption. Specifically it assumes that the probability distributions for the future states of each node are conditionally independent with respect to the previous state of the system. i.e. for the \textit{system} $Z$ which contains $n$ \textit{elementary mechanisms}:

\begin{equation}
\label{eq:cond_independence}
p(Z_{t+1}|Z_t = X) = \prod \limits_{i=1}^{n} p(z^i_{t+1}|Z_t=X)
\end{equation}
where we have reduced the probability distribution for future states of $Z$ to the products of the distributions for each \textit{elementary mechanism} in $Z$. Now, we use the fact that $p(z^i_{t+1}=1|Z_t=X)+p(z^i_{t+1} =0|Z_t=X) =1$ to conclude that if we know $p(z^i_{t+1}=1|Z_t=X)$ for all $i$, then we have all the information required to compute $p(Z_{t+1}|Z_t = X)$
...
The motivation behind \textit{distribution generators} is that they contain the same information as in the distribution itself, while occupying logarithmically less space. Specifically, a distribution of length $2^n$ will reduce to a \textit{distribution generator} of length $n$.

Consider the transition probability matrix $\mathbf{M}$. As is $M_{ij} = p(Z_{t+1} = j| Z_t = i), \forall i,j \in \mathbb{S}(Z)$, this implies that each row of $\mathbf{M}$ is $\mathbf{M}_i = p(Z_{t+1}|Z_t=i)$ matches our definition of probability distribution. However, we have just shown how such a distribution can be reduced to its generator, which takes up much less space. Thus we condense the matrix $\mathbf{M}$ into the \textit{condensed matrix} $\mathbf{T}$.
...
We demonstrate transforming the transition probability matrix into the \textit{condensed matrix} in equation \ref{eq:matrix_tranform_demo}. In practice this is done by multiplying $\mathbf{M}$ to the $2^n \times n$ matrix whose rows are the elements of $\mathbb{S}(Z)$ appearing in the same order that they apply in $\mathbf{M}$.
